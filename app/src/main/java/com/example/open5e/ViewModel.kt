    ...
    fun fetchMonstersPage(
        ...
        viewModelScope.launch {
            try {
                ...
                // lazy load: carregar páginas da API até termos itens suficientes para preencher a página desejada
                while (filtered.size <= desiredStart && monstersMoreAvailable) {
                    val response = apiService.getMonsters(monstersNextApiPage)
                    if (response.isSuccessful) {
                        val body = response.body()
                        if (body != null) {
                            // adiciona e remove duplicatas pelo nome
